...
# Check events
kubectl get events -n dclaw-research --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-research deployment/dclaw-research-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-research-db-credentials -n dclaw-research
   ```
