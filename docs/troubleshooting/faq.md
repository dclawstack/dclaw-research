...
### How do I update DClaw Research?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp research --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Research without Kubernetes?

Yes, for local development:

```bash
cd dclaw-research/frontend && npm run dev
cd dclaw-research/backend && uvicorn dclaw_research.main:app --reload
```
